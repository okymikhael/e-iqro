        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800 bg-opacity-90">
            ...
            @if(is_array($field) && array_key_first($field) == 'radio')
                ...
                <label class="inline-flex items-center text-gray-600 dark:text-gray-400 mb-4 mr-3">
                    <input type="radio"
                        ...
                        wire:model="{{$key}}" name="{{$key}}" value="{{$v}}" />
            ...
            @elseif(is_array($field) && array_key_first($field) == 'select')
                <label class="block text-sm mb-4">
                    ...
                    <select wire:model="{{$key}}" name="{{$key}}" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" id="">
            ...
            @elseif(is_array($field) && array_key_first($field) == 'button')
                <label class="block text-sm mb-4">
                    <div>
                        <button
                            class="flex items-right justify-between w-34 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-{{$field['button'][0]}}-600 border border-transparent rounded-lg active:bg-{{$field['button'][0]}}-600 hover:bg-{{$field['button'][0]}}-700 focus:outline-none focus:shadow-outline-{{$field['button'][0]}}">
            ...
            @else
                <label class="block text-sm mb-4">
                    ...
                    <input
                        ...
                        wire:model="{{$key}}" type="{{$field}}" name="{{$key}}" />
                </label>
                @error($key)
                <span class="text-xs text-red-700" id="passwordHelp">
                    {{ $message }}
